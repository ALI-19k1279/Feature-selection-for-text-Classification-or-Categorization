...
with open('simple wikipedia.txt', "r",errors='ignore') as f:
        ...
        #print(text)
        data.append(('simple wikipedia.txt',text))
        dict={'Document':data[0][0],
                'Tokens':data[0][1],
                }
        # file_exists = os.path.isfile('wikepedia.csv')
        with open('wikepedia.txt','w',encoding='utf-8') as f:
            for token in data[0][1]:
                 f.write("%s\n" % token)
            print('done')
